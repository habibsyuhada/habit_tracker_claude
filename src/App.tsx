...
const TABS: { type: TaskType; label: string; icon: string }[] = [
...
const EMPTY_HINT: Record<TaskType, string> = {
  ...
  return (
    <div className="app">
      ...
      <main className="task-list">
        {visible.length === 0 && (
          <div className="empty">
            <div className="empty-icon">{TABS.find((t) => t.type === tab)?.icon}</div>
            <p>{EMPTY_HINT[tab]}</p>
        ...
        {tab === 'todo' && doneTodoCount > 0 && (
          ...
          >
            {showDoneTodos ? 'Sembunyikan' : 'Tampilkan'} yang selesai ({doneTodoCount})
          </button>
        )}
      </main>
